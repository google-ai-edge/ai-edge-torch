...
from ai_edge_torch._convert.fx_passes import optimize_layout_transposes_pass
import torch
...
def partition(graph_module: torch.fx.GraphModule):
  ...
  for node in list(graph.nodes):
    ...
    if optimize_layout_transposes_pass.layout_check.must_be_nhwc(node):
      # If the node has must_be_nhwc equals true, mark this node as NHWC

      optimize_layout_transposes_pass.layout_mark.mark_as_nhwc_node(node)
    elif optimize_layout_transposes_pass.layout_check.can_be_nhwc(node):
      ...
      layout_sensitive_inputs = optimize_layout_transposes_pass.layout_check.get_layout_sensitive_inputs(
          node
      )

      should_be_nhwc = any(
          map(
              optimize_layout_transposes_pass.layout_mark.is_nhwc_node,
              layout_sensitive_inputs,
          )
      )
      for input_node in layout_sensitive_inputs:
        if not optimize_layout_transposes_pass.layout_mark.is_nhwc_node(
            input_node
        ) and not optimize_layout_transposes_pass.layout_check.is_4d(
            input_node
        ):
