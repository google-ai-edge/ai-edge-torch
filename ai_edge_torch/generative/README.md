...
* The conversion, and serialization process is unoptimized for LLMs. It requires keeping multiple copies of the weights in memory for transformations, and serialization/deserialization. To ensure a smooth conversion flow, we suggest you either try Colab Pro, or run the conversion on a beefy Linux workstation (or Cloud instance) with RAM over 32GB (the higher the better).
