...
from ai_edge_torch.generative.examples.experimental.gemma3 import gemma3_gpu
...
def main(_):
  if _MODEL_SIZE.value == '1b':
    pytorch_model = gemma3_gpu.build_model_1b(
        _CHECKPOINT_PATH.value, kv_cache_max_len=_KV_CACHE_MAX_LEN.value
    )
    config = pytorch_model.config
  elif _MODEL_SIZE.value == '4b':
    pytorch_model = gemma3_gpu.build_model_4b(
        _CHECKPOINT_PATH.value, kv_cache_max_len=_KV_CACHE_MAX_LEN.value
    )
