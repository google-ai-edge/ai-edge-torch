...
class Decoder2(gemma2.Gemma2):
  ...
  def forward(
    ...
    if mask is None:
      ...
      kv_cache_max_len = kv_cache.caches[0].k_cache.size(
          1 if kv_cache.caches[0].kv_layout == kv_utils.KV_LAYOUT_DEFAULT else 2
      )
      embeds_len = input_embeds.shape[1]
      mask = torch.zeros(embeds_len, kv_cache_max_len)
...
def build_decoder2(checkpoint_path: str) -> torch.nn.Module:
