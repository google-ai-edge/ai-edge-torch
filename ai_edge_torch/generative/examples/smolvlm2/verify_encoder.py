...
def main(_):
  ...
  original_vision_model = original_model.eval().model.vision_model
  ...
  outputs_reauthored = reauthored_model.siglip_encoder.forward(
      pixel_values=pixel_values
  )
  logging.info("outputs_reauthored: %s", outputs_reauthored)
  logging.info("outputs_reauthored: %s", outputs_reauthored.shape)

  full_reauthored_model = smolvlm2.build_model(
      reauthored_checkpoint,
      custom_loader=None,
      mask_cache_size=256,
  )
  outputs_reauthored_full = full_reauthored_model.connector.forward(
      outputs_reauthored
  )
  logging.info("outputs_reauthored_full: %s", outputs_reauthored_full)
  logging.info("outputs_reauthored_full: %s", outputs_reauthored_full.shape)
