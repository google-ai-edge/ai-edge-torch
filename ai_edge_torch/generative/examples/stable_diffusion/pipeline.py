...
def run_pytorch_pipeline(
  ...
  # Text embedding.
  cond_tokens = tokenizer.encode(prompt)
  clip_model = clip.CLIP(clip.get_model_config())
  loader = stable_diffusion_loader.ClipModelLoader(clip_ckpt, clip.TENSOR_NAMES)
  loader.load(clip_model, strict=False)
  cond_context = clip_model(torch.Tensor([cond_tokens]))

  uncond_tokens = tokenizer.encode(uncond_prompt)
  uncond_context = clip_model(torch.Tensor([uncond_tokens]))
  ...
  diffusion_loader = stable_diffusion_loader.DiffusionModelLoader(
      diffusion_ckpt, diffusion.TENSOR_NAMES
  )
  ...
  decoder_loader = stable_diffusion_loader.AutoEncoderModelLoader(
      decoder_ckpt, decoder.TENSOR_NAMES
  )
  ...
  if input_image:
    ...
    latents = torch.Tensor(np.random.normal(size=noise_shape).astype(np.float32))
    latents *= sampler.initial_scale
  ...
  for i, timestep in enumerate(timesteps):
    ...
    output = diffusion_model(input_latents, context, time_embedding)
    output_cond, output_uncond = np.split(output, 2, axis=0)
    output = cfg_scale * (output_cond - output_uncond) + output_uncond
    latents = sampler.step(latents, output)

  # Image decoding.
  images = decoder_model(latents)
  images = util.rescale(images, (-1, 1), (0, 255), clamp=True)
  images = util.move_channel(images, to='last').detach().numpy()
  if not os.path.exists(output_path):
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
  Image.fromarray(images[0].astype(np.uint8)).save(output_path)
...
if __name__ == '__main__':
  ...
  #   Tokenizer(args.tokenizer_vocab_dir),
  #   args.clip_ckpt,
  #   args.diffusion_ckpt,
  #   args.decoder_ckpt,
  #   prompt=args.prompt,
