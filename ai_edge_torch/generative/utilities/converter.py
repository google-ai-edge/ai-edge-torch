...
def define_conversion_flags(
  ...
  flags.DEFINE_bool(
      'use_magic_numbers',
      False,
      'If true, magic numbers are used for kv_cache_max_len and '
      'prefill_seq_lens to support dynamic shapes on GPU. kv_cache_max_len and '
      'the last prefill_seq_lens are used to calculate magic numbers.',
  )
  return flags
...
def get_mask_cache_size_from_flags() -> int:
  ...
  if flags.FLAGS.use_magic_numbers:
    return 0
  if flags.FLAGS.use_magic_numbers:
