...
from ai_edge_torch.generative.utilities.model_builder import GenerationConfig
import torch
...
class ModelWrapper(torch.nn.Module):
  ...
  def __init__(self, model: torch.nn.Module):
    ...
    super().__init__()
    self.model = model
    self.gen_config = GenerationConfig(output_logits_on_prefill=True)
...
class ReauthoredModelWrapper(ModelWrapper):
  ...
  def _forward_with_kv_cache(
    ...
    if (
        self.gen_config is not None
        and not self.gen_config.output_logits_on_prefill
    ):
    ...
    if pixel_values is None:
      output = self.model.forward(tokens, input_pos, kv_cache, self.gen_config)
    else:
