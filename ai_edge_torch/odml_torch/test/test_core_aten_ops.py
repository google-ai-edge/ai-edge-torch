...
class TestCoreAtenOps(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      ("aten_elu_0", torch.ops.aten.elu.default, (rnd(torch.float32, (10, 10), min_v=-1.0, max_v=1.0),), dict()),
      ("aten_elu_1", torch.ops.aten.elu.default, (rnd(torch.float32, (10, 10), min_v=-1.0, max_v=1.0), 1.5), dict()),
      ("aten_elu_2", torch.ops.aten.elu.default, (rnd(torch.float32, (10, 10), min_v=-1.0, max_v=1.0), 1.5, 1.5), dict()),
      ("aten_elu_3", torch.ops.aten.elu.default, (rnd(torch.float32, (10, 10), min_v=-1.0, max_v=1.0), 1.5, 1.5, 1.5), dict()),
      ...
      ("aten_hann_window_default_0", torch.ops.aten.hann_window.default, (10,), dict()),
      ("aten_hann_window_periodic_0", torch.ops.aten.hann_window.periodic, (10, True,), dict()),
      ("aten_hann_window_periodic_1", torch.ops.aten.hann_window.periodic, (10, False,), dict()),
      ...
      ("aten_topk_dim-1_largest", torch.ops.aten.topk, (rnd(torch.float32, (4, 6)), 3), dict(dim=-1, largest=True, sorted=True)),
      ("aten_topk_dim0_smallest", torch.ops.aten.topk, (rnd(torch.float32, (4, 6)), 2), dict(dim=0, largest=False, sorted=True)),
      ("aten_topk_dim1_largest", torch.ops.aten.topk, (rnd(torch.float32, (3, 4, 5)), 2), dict(dim=1, largest=True, sorted=True)),
      ("aten_topk_dim-1_smallest", torch.ops.aten.topk, (torch.arange(0, 12, dtype=torch.float32).reshape(4, 3), 2), dict(dim=-1, largest=False, sorted=True)),
      ("aten_topk_dim0_largest", torch.ops.aten.topk, (torch.arange(0, 12, dtype=torch.float32).reshape(4, 3), 2), dict(dim=0, largest=True, sorted=True)),
