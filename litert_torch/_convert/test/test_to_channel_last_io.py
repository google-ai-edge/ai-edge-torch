...
class TestToChannelLastIO(googletest.TestCase):
  ...
  def test_no_transformations(self):
    ...
    y = litert_torch.to_channel_last_io(Identity())(x)
    self.assertEqual(y.shape, (1, 3, 10, 10))

  def test_args(self):
    x = torch.rand(1, 10, 10, 3)
    y = litert_torch.to_channel_last_io(Identity(), args=[0])(x)
    self.assertEqual(y.shape, (1, 3, 10, 10))

  def test_outputs(self):
    x = torch.rand(1, 3, 10, 10)
    y = litert_torch.to_channel_last_io(Identity(), outputs=[0])(x)
    self.assertEqual(y.shape, (1, 10, 10, 3))

  def test_args_outputs(self):
    x = torch.rand(1, 10, 10, 3)
    y = litert_torch.to_channel_last_io(Identity(), args=[0], outputs=[0])(x)
    self.assertEqual(y.shape, (1, 10, 10, 3))
  ...
  def test_chained_wrappers(self):
    ...
    m = Identity()
    m = litert_torch.to_channel_last_io(m, args=[0])
    m = litert_torch.to_channel_last_io(m, outputs=[0])
  ...
  def test_list_args(self):
    ...
    y = litert_torch.to_channel_last_io(Add(), args=[0, 1])(*x)
    self.assertEqual(y.shape, (1, 3, 10, 10))
  ...
  def test_list_outputs(self):
    ...
    x = torch.rand(1, 3, 10, 10)
    y = litert_torch.to_channel_last_io(TwoIdentity(), outputs=[0])(x)
