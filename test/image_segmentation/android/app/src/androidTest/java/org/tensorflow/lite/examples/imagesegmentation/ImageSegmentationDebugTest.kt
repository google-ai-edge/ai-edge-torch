...
class ImageSegmentationDebugTest {
    companion object {
        ...
        private val STORAGE_FOLDER = File(
    ...
    private fun segmentWithTflite(image: Bitmap, imageRotation: Int) {
        ...
        val imageProcessor =
                ImageProcessor.Builder().add(ResizeOp(H, W, ResizeOp.ResizeMethod.BILINEAR))
                        .add(Rot90Op(-imageRotation / 90)).add(NormalizeOp(0.0f, 255.0f)).build()
    ...
    private fun runInference(
            interpreter: InterpreterApi, preprocessBuffer: ByteBuffer
    ): InferenceData {
        var W = 0
        var H = 0
        var C = 0
    ...
    fun saveBitmapOnStorage(tensorBuffer: TensorBuffer, filename: String) {
        ...
        for (i in 0 until tensorBuffer.shape[0] * tensorBuffer.shape[1]) {
            ...
            val pixel = if (tensorBuffer.dataType == DataType.FLOAT32)
                (255 * tensorBuffer.getFloatValue(i)) else tensorBuffer.getIntValue(i)
