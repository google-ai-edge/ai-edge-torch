...
class OverlayView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
  ...
  override fun draw(canvas: Canvas) {
    super.draw(canvas)
    scaleBitmap?.let { canvas.drawBitmap(it, 0f, 0f, null) }
  }

  fun setResults(segmentResult: List<Segmentation>?, imageHeight: Int, imageWidth: Int) {
    if (segmentResult != null && segmentResult.isNotEmpty()) {
      val colorLabels =
        segmentResult[0].coloredLabels.mapIndexed { index, coloredLabel ->
          ColorLabel(index, coloredLabel.getlabel(), coloredLabel.argb)
        }
      ...
      for (i in maskArray.indices) {
        ...
        val colorLabel = colorLabels[maskArray[i].toInt()].apply { isExist = true }
        val color = colorLabel.getColor()
        pixels[i] = color
      }

      val image =
        Bitmap.createBitmap(pixels, maskTensor.width, maskTensor.height, Bitmap.Config.ARGB_8888)
  ...
  data class ColorLabel(
    ...
    fun getColor(): Int {
      ...
      else Color.argb(ALPHA_COLOR, Color.red(rgbColor), Color.green(rgbColor), Color.blue(rgbColor))
    }
  }
}
